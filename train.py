...
if __name__ == '__main__':
    ...
    #加载训练集
    dataset_train = datasets.ImageFolder('../data/LFW_aligned', transform=train_transform)
    #print(dataset_train)
    ...
    # optionally resume from a checkpoint 可选择性地从检查点恢复
    ...
    #======= train & validation & save checkpoint =======#  训练前的小设置
    DISP_FREQ = len(train_loader) // 1 # frequency to display training loss & acc  显示训练损失和acc的频率 100
    print(len(train_loader))
    NUM_EPOCH_WARM_UP = NUM_EPOCH // 25  # use the first 1/25 epochs to warm up
    NUM_BATCH_WARM_UP = len(train_loader) * NUM_EPOCH_WARM_UP  # use the first 1/25 epochs to warm up
    batch = 0  # batch index

    #训练、验证和保存检查点(使用第一个1/25 epoch进行预热——逐步将LR增加到初始值，确保稳定收敛)
    for epoch in range(NUM_EPOCH): # start training process
        ...
        #print("Epoch {}/{}, Evaluation: LFW Acc: {},CFP_FF Acc: {},CFP_FP Acc:{},Agedb Acc:{},CPLFW Acc:{},VGG2_FP Acc:{},CALFW Acc:{}".format(epoch + 1, NUM_EPOCH, accuracy_lfw, accuracy_cfp_ff, accuracy_cfp_fp, accuracy_agedb, accuracy_cplfw, accuracy_vgg2_fp, accuracy_calfw))
